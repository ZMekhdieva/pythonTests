...
def test_16(browser, pdf_file):
    """
    Сценарий 15. Выгрузка отчета "Итоговая повестка Градсовет"
        ...
        Нажать кнопку "Отчет Итоговая повестка Градсовет"
    ...
    with allure.step('Шаг 3. МИО. Итоговая повестка'):
        try:
            browser.hover_and_click("html/body/nav/div/div[2]/ul/li[1]/a", ".//*[@id='final_notice']")
            # browser.get('http://10.5.127.12/web/mio_final_notice')
            time.sleep(3)
            browser. set_date(".//*[@id='noticeDate']")
            time.sleep(5)
            browser.xpath(".//*[@id='finalNoticeLandsWithCityCouncilSignReportXls']").click()
            xp = ".//*[@id='close']"
